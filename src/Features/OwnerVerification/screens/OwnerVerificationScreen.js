...
import { styles } from '../stylesheets/OwnerVerification.style';
...
const OwnerVerificationScreen = ({ navigation }) => {
  ...
  const STATUS_CONFIG = {
    not_submitted: {
      ...
      color: '#9CA3AF',
      label: t('not_submitted_status'),
    },
    pending: {
      icon: 'schedule',
      color: colors.warning,
      label: t('under_review_status'),
    },
    approved: {
      icon: 'check-circle',
  ...
  const handleSubmit = () => {
    if (!idFrontUploaded || !idBackUploaded) {
      Alert.alert(
        'Missing Documents',
        'Please upload both sides of your government ID.',
      );
      return;
    }
    if (!declarationAccepted) {
      Alert.alert(
        ...
        'Please accept the ownership declaration.',
      );
      return;
    }
    setSubmitted(true);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['left', 'right']}>
      {/* Header */}
      <CustomHeader
        title="owner_verification_title"
      ...
      {overallStatus !== 'not_submitted' && (
        <View
          style={[styles.statusBanner, { backgroundColor: cfg.color + '18' }]}
        >
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {currentStep === 0 && (
          <View>
            ...
            <View style={styles.ownerTypeRow}>
              {OWNER_TYPES.map(type => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.typeChip,
                    ownerType === type && styles.typeChipActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.typeChipText,
                      ownerType === type && styles.typeChipTextActive,
                    ]}
                  >
                    {type}
                  </Text>
            ...
            <TouchableOpacity
              style={[
                styles.uploadBox,
                idFrontUploaded && styles.uploadBoxDone,
              ]}
              onPress={() => simulateUpload(setIdFrontUploaded, 'ID Front')}
            >
              <Icon
                name={idFrontUploaded ? 'check-circle' : 'add-photo-alternate'}
                size={moderateScale(32)}
                color={idFrontUploaded ? colors.success : '#9CA3AF'}
              />
              <Text
                style={[
                  styles.uploadText,
                  idFrontUploaded && { color: colors.success },
                ]}
              >
                {idFrontUploaded
                  ? t('id_front_uploaded')
                  : t('tap_upload_id_front')}
              </Text>
            </TouchableOpacity>

            <Text style={styles.fieldLabel}>{t('government_id_back')}</Text>
            <TouchableOpacity
              style={[styles.uploadBox, idBackUploaded && styles.uploadBoxDone]}
              onPress={() => simulateUpload(setIdBackUploaded, 'ID Back')}
            >
              <Icon
                name={idBackUploaded ? 'check-circle' : 'add-photo-alternate'}
                size={moderateScale(32)}
                color={idBackUploaded ? colors.success : '#9CA3AF'}
              />
              <Text
                style={[
                  styles.uploadText,
                  idBackUploaded && { color: colors.success },
                ]}
              >
                {idBackUploaded
                  ? t('id_back_uploaded')
                  : t('tap_upload_id_back')}
              </Text>
            </TouchableOpacity>

            <View style={styles.requirementsList}>
              {[
                t('clear_unobstructed_photo'),
                t('all_corners_visible'),
                t('not_expired'),
                t('file_size_limit'),
              ].map(req => (
        ...
        {currentStep === 1 && (
          <View>
            ...
            <Text style={styles.stepSubtitle}>
              {t('ownership_declaration_subtitle')}
            </Text>

            <View style={styles.declarationCard}>
              <Text style={styles.declarationTitle}>Ownership Declaration</Text>
              <Text style={styles.declarationText}>
                I, the undersigned, hereby declare that I am the legal owner or
                authorized agent of all trailers I list on the Trailors
                platform. I confirm that:{'\n\n'}• All listed trailers are
                legally registered and roadworthy{'\n'}• I have the right to
                rent out the listed trailers{'\n'}• All information provided is
                accurate and truthful{'\n'}• I will comply with all platform
              ...
              >
                ...
                <Text style={styles.acceptText}>
                  I accept the ownership declaration and confirm all information
                  is accurate
                </Text>
              </TouchableOpacity>
            </View>
            ...
            <Text style={styles.fieldLabel}>
              {t('trailer_registration_optional')}
            </Text>
            <TouchableOpacity
              style={[styles.uploadBox, regDocUploaded && styles.uploadBoxDone]}
              onPress={() =>
                simulateUpload(setRegDocUploaded, 'Registration Document')
              }
            >
              <Icon
                name={regDocUploaded ? 'check-circle' : 'upload-file'}
                size={moderateScale(32)}
                color={regDocUploaded ? colors.success : '#9CA3AF'}
              />
              <Text
                style={[
                  styles.uploadText,
                  regDocUploaded && { color: colors.success },
                ]}
              >
                {regDocUploaded
                  ? t('document_uploaded')
                  : t('tap_upload_registration')}
              </Text>
          ...
          <View>
            ...
            <View style={styles.reviewCard}>
              ...
              <View style={styles.reviewRow}>
                <Icon
                  name="assignment-turned-in"
                  size={moderateScale(18)}
                  color={declarationAccepted ? colors.success : colors.warning}
                />
                ...
                >
                  {declarationAccepted
                    ? t('declaration_accepted')
                    : t('declaration_not_accepted')}
                </Text>
              </View>
              <View style={[styles.reviewRow, { borderBottomWidth: 0 }]}>
                ...
                <Text style={styles.reviewValue}>
                  {regDocUploaded
                    ? t('registration_uploaded')
                    : t('registration_not_uploaded')}
                </Text>
              </View>
            </View>
            ...
            <View style={styles.reviewNote}>
              <Icon
                ...
                color={colors.primary}
              />
              <Text style={styles.reviewNoteText}>{t('review_note')}</Text>
            </View>
