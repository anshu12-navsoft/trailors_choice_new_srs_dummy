...
// ── Media Upload Slot ─────────────────────────────────────────────────────
const MediaSlot = ({ uri, onPress, iconName, iconOverlay, label, selectedText }) => (
 <Pressable
   onPress={onPress}
 ...
 >
   {uri ? (
     ...
     <>
       <Icon name="check-circle" size={moderateScale(30)} color="#3B5BDB" />
       <Text style={media.selectedText} numberOfLines={2}>{selectedText}</Text>
     </>
   ) : (
     <>
 ...
 return (
   <View style={step.container}>
     ...
     <MediaSlot
       ...
       selectedText={
         form.mediaVideos?.length
           ? `${form.mediaVideos.length} video${form.mediaVideos.length > 1 ? 's' : ''} selected\n${form.mediaVideos[0].name ?? form.mediaVideos[0]}`
           : null
       }
     />
     ...
     <MediaSlot
       ...
       selectedText={
         form.mediaDocuments?.length
           ? `${form.mediaDocuments.length} file${form.mediaDocuments.length > 1 ? 's' : ''} selected\n${form.mediaDocuments[0].name ?? form.mediaDocuments[0]}`
           : null
       }
     />
...
const buildFormData = (form, isDraft = false) => {
 ...
 (form.mediaPhotos || []).forEach((uri, i) => {
   ...
   fd.append('files', { uri, type: `image/${ext}`, name: `photo_${i}.${ext}` });
 });
 (form.mediaVideos || []).forEach((v, i) => {
   const uri = typeof v === 'string' ? v : v.uri;
   const name = typeof v === 'object' ? v.name : `video_${i}.${uri.split('.').pop()?.split('?')[0] || 'mp4'}`;
   const type = typeof v === 'object' ? v.type : `video/${uri.split('.').pop()?.split('?')[0] || 'mp4'}`;
   fd.append('files', { uri, type, name });
 });
 (form.mediaDocuments || []).forEach((d, i) => {
   const uri = typeof d === 'string' ? d : d.uri;
   const name = typeof d === 'object' ? d.name : `doc_${i}.${uri.split('.').pop()?.split('?')[0] || 'pdf'}`;
   const type = typeof d === 'object' ? d.type : `application/${uri.split('.').pop()?.split('?')[0] || 'pdf'}`;
   fd.append('files', { uri, type, name });
 });
